...
(async () => {
	...
	json = (await fetch("top.csv").then((res) => res.text()))
		.split("\n")
		.map((i) => i.split(",")[1]);
	...
	if (request.type === "search") {
		...
		if (request.searchType === "combined") {
			let results = Object.keys(request.searchTypes)
				...
				.map(([key, val]) => [...val.map((item) => ({ ...item, type: key }))])
				.flat();
			items = results;
		} else {
		...
		const fuse = new Fuse(items, {
			includeScore: true,
			keys: ["url", "title"],
			threshold: request.query.length > 10 ? 0.5 : 0.4,
		});
		let out = fuse
		...
		if (request.query.length > 5) {
			let item = json.find((i) =>
				i.includes(request.query.trim().toLowerCase())
			);
			if (item) {
				out = [
					{
		...
		(async () => {
			try {
				...
				} else if (d.querySelector(".g h3") && d.querySelector(".g a")) {
...
async function updateStuff() {
	...
	stuff = {
		...
		downloads: (
			await new Promise((res) => chrome.downloads.search({}, res))
		).filter((i) => i.filename),
	};
	return;
}
...
async function main(_tab) {
	...
	let { id: tab } =
		...
		(await new Promise((res) =>
			chrome.tabs.query({ currentWindow: true, active: true }, ([t]) => res(t))
		));
	...
	let code = interpolate(
		{
			...
			items: {
				combined: [
					...
					...stuff.history
						.slice(0, 10)
						.map((i) => ({ ...i, type: "bookmarks" })),
				],
				bookmarks: stuff.bookmarks.slice(0, 30),
				history: stuff.history.slice(0, 30),
			},
			itemsLength: {
				downloads: stuff.downloads.length,
				combined: Object.values(stuff).flat().length,
				history: stuff.history.length,
				bookmarks: stuff.bookmarks.length,
				tabs: stuff.tabs.length,
			},
		},
		[
...
async function answer(q) {
	...
	var el =
		...
		[...d.querySelectorAll("div, span")]
			...
			?.slice(-1)?.[0]
			?.innerText?.replace(
				/^(?:description|calculator result | featured snippet from the web)/i,
				""
			) ||
	...
	if (
		text?.startsWith("Did you mean") ||
		text?.startsWith("In order to show you the most relevant results,")
	) {
		return;
	}
